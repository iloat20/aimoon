...
Returns ~180 daily bars with OHLCV + pct_change.
"""
...
class KlineCollector(DataCollector[KlineData]):
    ...
    def _ak_hist(self, symbol: str):
        ...
        start = (datetime.now() - timedelta(days=self._days * 3)).strftime("%Y%m%d")
    ...
    def _ak_daily(self, symbol: str):
        ...
        return retry_on_connection(ak.stock_zh_a_daily, symbol=f"{prefix}{symbol}", adjust="qfq")
    ...
    async def _fetch_tencent(self, symbol: str) -> KlineData | None:
        ...
        for row in day_data:
            ...
            # 第一根K线无前收盘价，涨跌幅设为 None
            pct = ((close_f - prev_close) / prev_close * 100) if prev_close else None
            prev_close = close_f
