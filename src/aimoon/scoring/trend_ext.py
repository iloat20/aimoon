    ...
    *,
    ...
    if not any(pd.isna(v.iloc[-1]) for v in [ma5, ma10, ma20, ma60]):
        ...
        if ma20.iloc[-1] > ma60.iloc[-1]:
            bullish_pairs += 1
        if bullish_pairs == 3:
            signals.append(Signal("ma_align_bull", "均线完美多头排列", +3, category="reversal"))
        elif bullish_pairs == 2:
            signals.append(Signal("ma_align_partial", "均线偏多排列", +1, category="reversal"))
        elif bullish_pairs == 0:
            signals.append(Signal("ma_align_bear", "均线空头排列", -2, category="reversal"))
    ...
    if above_ma20 and above_ma60:
        signals.append(Signal("above_ma20_60", "价格在MA20和MA60上方", +2, category="reversal"))
    elif above_ma20:
        signals.append(Signal("above_ma20", "价格在MA20上方", +1, category="reversal"))
    elif not above_ma20 and not above_ma60:
        signals.append(Signal("below_ma20_60", "价格在MA20和MA60下方", -2, category="reversal"))
    ...
    if adx_val > 25:
        if plus_di > minus_di:
            signals.append(
                Signal("adx_bull_trend", f"ADX强多头趋势({adx_val:.0f})", +2, category="reversal")
            )
        else:
            signals.append(
                Signal("adx_bear_trend", f"ADX强空头趋势({adx_val:.0f})", -2, category="reversal")
            )
    ...
    if len(hist) >= 5:
        ...
        if red_count >= 4:
            signals.append(
                Signal("macd_red_streak", f"MACD连续{red_count}根红柱", +2, category="reversal")
            )
        elif green_count >= 4:
            signals.append(
                Signal("macd_green_streak", f"MACD连续{green_count}根绿柱", -2, category="reversal")
            )
    ...
    if len(ema20) >= 5:
        ...
        if slope > 2:
            signals.append(
                Signal("ema_slope_up", f"EMA20上升({slope:+.1f}%)", +1, category="reversal")
            )
        elif slope < -2:
            signals.append(
                Signal("ema_slope_down", f"EMA20下降({slope:+.1f}%)", -1, category="reversal")
            )
