    ...
    *,
        ...
        if not any(pd.isna(x) for x in [c_now, m_now, rsi_now, rsi_prev]):
            ...
            if mdd < -10 and c_now > m_now and rsi_now > rsi_prev and rsi_now < 60:
                signals.append(
                    Signal(
                        "reversal_confirmed",
                        f"确认反转(回撤{mdd:.0f}%,站上MA20,RSI{rsi_now:.0f})",
                        +4,
                        category="reversal",
                    )
                )
            ...
            if not pd.isna(r_now) and not pd.isna(r_5ago):
                if r_5ago < -5 and r_now > 2:
                    signals.append(
                        Signal(
                            "reversal_roc20_turn",
                            f"中期动量转正(ROC20:{r_5ago:+.1f}%→{r_now:+.1f}%)",
                            +3,
                            category="reversal",
                        )
                    )
    ...
    if len(close) >= 6:
        ...
        if down_days >= 3 and today_return > 2 and vol_ratio > 1.5:
            signals.append(
                Signal(
                    "reversal_vol_reversal",
                    f"连跌后放量大阳(+{today_return:.1f}%,量比{vol_ratio:.1f}x)",
                    +3,
                    category="reversal",
                )
            )
