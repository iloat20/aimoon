...
def score_kdj(ti: TechInd, *, code: str = "", ctx: dict | None = None) -> list[Signal]:
    ...
    if ti.kdj_golden_cross():
        signals.append(Signal("kdj_golden", "KDJ金叉", +1, category="reversal"))
    if ti.kdj_death_cross():
        signals.append(Signal("kdj_death", "KDJ死叉", -1, category="reversal"))
    if ti.kdj_oversold():
        signals.append(Signal("kdj_oversold", "KDJ超卖", +1, category="reversal"))
