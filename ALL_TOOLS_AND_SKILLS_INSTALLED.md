...
**主要功能**：
- ✅ 23 个设计命令（craft、shape、audit、critique 等）
- ✅ 7 个领域参考文件（typography、color、motion 等）
- ✅ 27 个反模式规则
- ✅ 自动触发机制
...
| 功能 | 工具 | 效果 |
|------|------|------|
| 需求分析 | Superpowers | ✅ 自动头脑风暴 |
| 计划制定 | Superpowers | ✅ 详细实现计划 |
| 代码实现 | Superpowers | ✅ TDD 自动执行 |
| 前端设计 | Impeccable | ✅ 专业级设计 |
| Token 节省 | RTK + Headroom | ✅ 70-95% 节省 |
...
/impeccable animate hero         # 添加动画
/impeccable colorize buttons     # 引入颜色
```
...
**流程**：
1. 你："我想添加用户认证功能"
2. Claude：自动触发 Superpowers brainstorming，询问需求细节
3. 你：回答问题，确认设计
4. Claude：自动触发 Superpowers writing-plans，生成详细计划
5. 你：审查计划，说"开始"
6. Claude：自动触发 Superpowers executing-plans，使用子代理执行每个任务
7. 每个任务：自动触发 Superpowers test-driven-development
8. 任务完成后：自动触发 Superpowers requesting-code-review
9. 所有任务完成后：自动触发 Superpowers finishing-a-development-branch
...
**流程**：
1. 你："我想设计一个登录页面"
2. Claude：自动触发 Impeccable shape，规划 UX/UI
3. 你：确认设计方向
4. Claude：自动触发 Impeccable craft，生成完整设计
5. 你：审查并提供反馈
6. Claude：自动触发 Impeccable polish，最终优化
7. 你：发布设计
...
**流程**：
1. 你："这个测试失败了"
2. Claude：自动触发 Superpowers systematic-debugging
3. 4 阶段根因分析
4. 提供修复建议
5. 修复后：自动触发 Superpowers verification-before-completion
...
**流程**：
1. 你："请审查这个 PR"
2. Claude：自动触发 Superpowers receiving-code-review
3. 按严重程度报告问题
4. 提供改进建议
...
### Superpowers
- ✅ `docs/superpowers_installation.md` - 安装指南
...
### Impeccable
- ✅ `docs/impeccable_installation.md` - 安装指南
- ✅ `IMPECCABLE_INSTALLED.md` - 安装总结
...
- ✅ `ALL_TOOLS_INSTALLED.md` - 所有工具安装完成
- ✅ `ALL_TOOLS_AND_SKILLS_INSTALLED.md` - 本文件
...
**问题：技能没有加载**
- 检查技能文件：`ls ~/.claude/skills/impeccable/`
- 确保 SKILL.md 存在
- 重启 Claude Code
...
2. **开始新功能**
   - 描述需求，触发 Superpowers brainstorming
   - 享受自动化开发流程

3. **设计前端页面**
   - 描述设计需求，触发 Impeccable shape
   - 享受专业级前端设计
