...
def run_optimized_hybrid_backtest():
    """运行优化的混合评分回测"""

    print("=" * 70)
    print("优化的混合评分回测 - 降低入场阈值")
    print("=" * 70)
    ...
    for i, stock in enumerate(top_stocks[:10], 1):
        print(f"      {i}. {stock.code} ({stock.name}) - 混合分数: {stock.hybrid_score}")
    ...
    # 优化参数（基于回测结果）
    entry_threshold = 50.0  # 从 40 提高到 50，提高入场质量
    stop_loss_pct = 0.04    # 从 5% 降到 4%，收紧止损控制回撤
    take_profit_pct = 0.25  # 从 30% 降到 25%，提高盈亏比
    ...
    engine = EnhancedBacktestEngine(
        ...
        slippage=0.001,
        stamp_tax=0.0005,
        stop_loss_pct=stop_loss_pct,      # 优化：收紧止损
        take_profit_pct=take_profit_pct,  # 优化：降低止盈
        benchmark_code=cfg.benchmark_code,
        entry_threshold=entry_threshold,   # 优化：提高入场阈值
    ...
    print(f"   入场阈值: {entry_threshold} (优化后)")
    print(f"   止损: {stop_loss_pct:.2%} (优化后，原 {cfg.stop_loss_pct:.2%})")
    print(f"   止盈: {take_profit_pct:.2%} (优化后，原 {cfg.take_profit_pct:.2%})")
    ...
    if result.trade_count > 0:
        ...
        print("\n⚠️  未产生交易")
        print("   可能原因:")
        print(f"   1. 入场阈值仍然过高（当前: {entry_threshold}）")
        print("   2. 数据覆盖范围不足")
        print("   3. K 线数据质量问题")
    ...
    try:
        ...
        eq_path = os.path.join(cfg.output_dir, "optimized_equity_curve.png")
        dd_path = os.path.join(cfg.output_dir, "optimized_drawdown.png")
        mr_path = os.path.join(cfg.output_dir, "optimized_monthly_returns.png")

        plot_equity_curve(result.equity_curve, title="Optimized Hybrid Scoring Equity Curve", filepath=eq_path)
